$(document).ready(function() {
    ...
    var search_change = function() {
        ...
        $.getJSON('/api/movie_keyword?key=' + key, function(data) {
            for(var i=0; i<data['data'].length; i++) {
                var movie = $.parseJSON(data['data'][i]);

                var a = document.createElement('a');
                $(a).addClass('list-group-item');
                a.href = '/post/' + movie['id'];
                a.title = movie['title'];
                ...
                $('#search-data').append(a);
            }
        })
    };

    $('#search').bind('keyup keydown', search_change);
